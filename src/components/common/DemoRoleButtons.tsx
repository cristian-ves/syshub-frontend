...
import { ShieldCheck, GraduationCap, PenSquare, ArrowUpRight } from "lucide-react";
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {ROLES.map(({ role, label, description, icon: Icon, accent }) => (
                    <button
                        ...
                        className="group relative flex flex-col items-start gap-2 p-4 rounded-xl border-2 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:border-brand-blue dark:hover:border-blue-400 hover:shadow-lg hover:shadow-brand-blue/10 hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98] transition-all cursor-pointer text-left overflow-hidden"
                    >
                        <ArrowUpRight
                            size={16}
