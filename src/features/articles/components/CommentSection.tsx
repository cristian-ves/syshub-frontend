...
export const CommentSection: React.FC<CommentSectionProps> = ({ articleId, comments = [] }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await dispatch(addCommentThunk({ articleId, content: content })).unwrap();
            setContent('');
            toast.success('Comment published');
        } catch (error: any) {
            toast.error(error || 'Error publishing the comment');
        } finally {
            ...
            <div className="space-y-4">
                {comments.length === 0 ? (
                    <div className="text-center py-12 text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-900/30 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800">
                        ...
                        <p className="font-medium">There are no comments yet.</p>
                        <p className="text-sm mt-1">Be the first to share your opinion!</p>
                    </div>
                ) : (
